...
\author{
  \IEEEauthorblockN{Timothy E. Smith, PE}
  \IEEEauthorblockA{
...
\begin{enumerate}[leftmargin=*,nosep]
  ...
  \item \textbf{Implementation}: A working implementation using PyCardano with 660 passing tests, demonstrating practical feasibility.\footnote{Complementary to Veridian ACDC (Apr 2025) which uses off-chain VCs + optional ledger hash anchoring~\cite{veridian2025}.}
\end{enumerate}
...
\begin{tikzpicture}[scale=0.78, every node/.style={transform shape}]
  ...
  \node[archbox, fill=blue!8, minimum width=2.2cm] (ui1) at (-2.8,7.8) {Web\\Dashboard};
  \node[archbox, fill=blue!8, minimum width=2.2cm] (ui2) at (0,7.8) {Python\\API};
  ...
  \node[archbox, fill=green!8, minimum width=3cm] (app2) at (1.9,6) {tx\_utils\\Build / Transfer\\UTxO Selection};
...
\begin{tikzpicture}[scale=0.78, every node/.style={transform shape}]
  ...
  % Tokens
  \node[tokenbox, fill=blue!6] (lic) at (-3.2,3.6) {License NFT\\(CIP-68)\\[2pt]\scriptsize User Token (222)\\Ref Token (100)};
...
\textbf{Token 1---License NFT (CIP-68).} A non-fungible token representing a professional license, implemented using the CIP-68 datum metadata standard~\cite{cip68}. The licensee holds an immutable \emph{User Token} (asset label 222) in their personal wallet, while the issuing authority retains a \emph{Reference Token} (asset label 100) containing an inline datum with license type, jurisdiction, issue date, licensee identity, and a mutable \texttt{status} field. Revocation is implemented by updating the Reference Token's datum to \texttt{status: revoked}---requiring only the authority's signature and zero interaction with the licensee's wallet. Any verifier reads the Reference Token's inline datum to determine current license status, providing instant global revocation propagation.
...
\textbf{CIP-68 Architecture.} The CIP-68 pattern is central to the revocation and status update mechanism. Because the user holds only the immutable User Token while the authority controls the Reference Token datum, unilateral revocation is possible without consuming the user's UTxO---a critical requirement given that eUTxO spending requires the UTxO holder's private key. Future Plutus V3 migration enables richer on-chain datum validation logic without altering this fundamental custody separation~\cite{cip68}.

\subsection{Wallet Model}

The system defines four wallet roles with distinct permissions, as shown in Table~\ref{tab:wallets}. All wallets use CIP-1852 hierarchical deterministic key derivation from a 24-word BIP-39 mnemonic. To avoid centralizing user private keys into a server-side honeypot, the production architecture employs a \textbf{non-custodial model}: licensee and signer wallets connect via CIP-30 (dApp Connector), allowing professionals to sign transactions using standard Cardano browser wallets (e.g., Eternl, Nami, Lace). Only the authority wallet's keys are stored server-side under AES-256-GCM encryption.
...
Licensee & \texttt{licensee} & Hold license NFT, deposit sig/val tokens, pay dues & CIP-30 wallet \\
Signer & \texttt{signer} & Deposit sig + val tokens into work product validators & CIP-30 wallet \\
...
The system employs a hybrid approach: \textbf{native scripts} (Phase~1 simple scripts) handle minting policy enforcement at zero Plutus execution cost (${\sim}$0.17~ADA per transaction), while the \textbf{Signature Collection Validator} and \textbf{Dues Enforcement Contract} are Plutus V2 scripts that use redeemers and incur standard Plutus CPU/Memory execution fees (${\sim}$0.3--0.5~ADA per transaction). Fig.~\ref{fig:contracts} illustrates the smart contract hierarchy.

\begin{figure}[t]
\centering
\begin{tikzpicture}[scale=0.78, every node/.style={transform shape}]
  % Minting Policy
  \node[archbox, fill=blue!8, minimum width=4.8cm, minimum height=1.6cm] (policy) at (0,5.2) {Native Script Minting Policy\\[2pt]\scriptsize ScriptPubkey / ScriptAll\\Controls ALL token mint/burn};
...
\subsubsection{Signature Collection Validator} Manages multi-party document signing through three redeemer actions: \textsc{Collect} (any required signer deposits tokens), \textsc{Finalize} (authority signs when all deposits present), and \textsc{Reclaim} (original depositor reclaims if cancelled). To avoid eUTxO contention when multiple signers deposit simultaneously, each signer creates an independent UTxO at the script address containing their tokens and a datum identifying the work product. The authority's \textsc{Finalize} transaction then consumes all per-signer UTxOs atomically, eliminating the single-UTxO concurrency bottleneck inherent in shared-state designs.
...
payment_vk = PaymentVerificationKey.from_signing_key(
    payment_sk)
stake_child = hd_wallet.derive_from_path(
    "m/1852'/1815'/0'/2/0")
stake_vk = StakeVerificationKey.from_signing_key(
    StakeSigningKey(stake_child.xprivate_key[:32]))
...
The \texttt{tx\_utils} module provides six transaction-building functions: \texttt{estimate\_fee()}, \texttt{build\_mint\_tx()}, \texttt{build\_transfer\_tx()}, \texttt{build\_multisig\_tx()}, \texttt{submit\_tx()}, and \texttt{wait\_for\_confirmation()}. UTxO selection defaults to PyCardano's \texttt{LargestFirstSelector} with a 200,000 lovelace fee buffer, avoiding the fragmentation risks of a naive greedy algorithm when users accumulate many small signature token UTxOs. The \texttt{RandomImproveSelector} is available as an alternative for better UTxO set distribution. Maximum transaction size is 16,384 bytes.
...
blockchain\_minting\_policies & Authority minting policy registry \\
dues\_contracts & Dues enforcement contracts \\
dues\_payments & Dues payment transaction records \\
\bottomrule
\end{tabular}
\end{table}
...
The package exposes a comprehensive Python API for credential lifecycle management: wallet generation (\texttt{generate\_wallet}, \texttt{create\_authority\_wallet}), license minting (\texttt{mint\_license\_nft}, \texttt{build\_cip25\_metadata}), signature operations (\texttt{sign\_document}, \texttt{verify\_signature}), validity management (\texttt{mint\_validity\_token}, \texttt{check\_validity}, \texttt{renew\_validity}), and dues enforcement (\texttt{deploy\_dues\_contract}, \texttt{pay\_dues}). All functions are async and designed for integration with web frameworks, CLI tools, or custom dashboards.
...
Table~\ref{tab:costs} summarizes on-chain costs for key operations. Minting operations use native script execution (zero Plutus execution fees, ${\sim}$0.17~ADA). Validator operations (signature collection, dues enforcement, CIP-68 datum updates) use Plutus V2 execution and incur additional CPU/Memory fees (${\sim}$0.3--0.5~ADA total).
...
Revoke License (datum update) & $\sim$0.3 & 0 & $\sim$0.3\textsuperscript{$\dagger$} \\
\bottomrule
\end{tabular}
\vspace{2pt}
\scriptsize\textsuperscript{$\dagger$}Revocation via CIP-68 datum update modifies the authority-held Reference Token in place; the ${\sim}$2.0~ADA minUTxO deposit remains locked at the Reference Token UTxO. If the authority later burns the token entirely, the locked minUTxO is recovered, yielding a net refund of ${\sim}$1.7~ADA.\\[2pt]
\textit{Note: Hydra state-channel batching (2026 roadmap) reduces effective cost to sub-0.01 ADA for high-volume renewals.}
\end{table}
...
The Massachusetts board mints a license NFT (type=MD, specialty=Internal Medicine). The IMLC Commission mints compact license NFTs cross-referencing it. Real-time verification replaces the multi-day FCVS process. If Massachusetts takes an adverse action, the board updates the CIP-68 Reference Token datum to \texttt{status: revoked} in a single transaction---requiring only the board's authority key. The revocation cascade---from board action to nationwide effect---completes within the $\sim$5-minute Cardano finality window versus weeks through the NPDB~\cite{npdb2024,imlc2025}. Cross-jurisdiction reciprocity via cross-referenced NFTs can be optionally anchored to Veridian ACDC for eIDAS 2.0 interoperability~\cite{veridian2025}.
...
\begin{tikzpicture}[scale=0.75, every node/.style={transform shape}]
  ...
  \draw[seqarrow, dashed, red!60!black] (0,-6.6) -- node[above, font=\tiny, text=red!60!black] {8. Revoke: update ref datum} (3.2,-6.6);
\end{tikzpicture}
...
Revocation & Issuer list & Rev.\ registry & DID deactivation & CRL/OCSP & Protocol-level & \textbf{CIP-68 datum} \\
...
\textbf{Centralized revocation} is replaced by CIP-68 Reference Token datum updates under authority-controlled policies, propagating globally within $\sim$5 minutes versus days-to-weeks through current systems. The authority updates the datum unilaterally---no interaction with the licensee's wallet is required.
...
The four use case scenarios demonstrate practical applicability across the professions most affected by credential verification failures. With 660 passing tests across wallet management, token operations, smart contract deployment, and transaction building, the implementation demonstrates technical feasibility. Future work prioritizes full Plutus V3 on-chain validators, privacy-preserving credential references via Halo2 ZKP predicates, and W3C Verifiable Credentials interoperability. The architecture is positioned as a production complement to Veridian/Identus for public-record regulated professions (medical, legal, engineering).
...
Development of the prototype and preparation of this paper were assisted by Claude Code (Anthropic). All architectural decisions, technical claims, and editorial judgments are the sole responsibility of the author.
...
\bibitem{cip30}
Cardano Foundation, ``CIP-30: Cardano dApp-Wallet Web Bridge,'' 2022.
